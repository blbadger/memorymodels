...
#frozen_encoder = LanguageMixer(n_vocab, decoder_dim, 8, tokenized_length, n_heads=heads, kernel=kernel).float().to(device)
...
frozen_encoder = TruncatedModel(frozen_encoder, autoencoder=True)
model = AutoencodingMixer(n_vocab, encoder_dim, n_layers, tokenized_length, compression=compression, n_heads=heads, kernel=1, unroll=False, random=False, frozen_encoder=frozen_encoder, clm_encoder=False)
...
output_dir = f'{checkpoint_root}/fineweb_frozen_encoder_autoencoder\
_{encoder_dim}\
c{compression}\
_d{decoder_dim}\
_n{n_layers}\
...
safetensors.torch.load_model(model, '/home/bbadger/Desktop/fineweb_frozen_encoder_autoencoder_1024c1_d1024_n8_c512_b32x4/checkpoint-200000/model.safetensors')
