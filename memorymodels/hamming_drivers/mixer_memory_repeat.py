...
print (model)
frozen_encoder = model
frozen_encoder.model_blocks = model.encoderblocks
#frozen_encoder = TruncatedModel(model, autoencoder=False).encoder_blocks
model = AutoencodingMixer(n_vocab, encoder_dim, n_layers, tokenized_length, compression=compression, n_heads=heads, kernel=kernel, unroll=False, random=False, frozen_encoder=frozen_encoder, clm_encoder=False)
...
#safetensors.torch.load_model(model, '/home/azureuser/fineweb_c256x4_memory_mixer_k8_512c1_d1024_n16_c256_b64x2/checkpoint-200000/model.safetensors')
...
output_dir = f'{checkpoint_root}/fineweb_frozen_memenc_autoencoder\
_{encoder_dim}\
c{compression}\
_d{decoder_dim}\
_n{n_layers}\
...
safetensors.torch.load_model(model, '/home/azureuser/fineweb_frozen_memenc_autoencoder_1024c1_d1024_n8_c512_b64x2/checkpoint-200000/model.safetensors')
