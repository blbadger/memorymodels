...
from torch.quantization.observer import MinMaxObserver, MovingAverageMinMaxObserver, HistogramO
...
model = MemoryTransformer(vocab_size, decoder_dim, encoder_dim, n_layers, context_length, transformer_encoder=encoder_model, compression=compression, n_heads=n_heads, random=False)

my_qconfig = QConfig(
    activation=MovingAverageMinMax.with_args(dtype=torch.fp16),
    weight=default_observer.with_args(dtype=torch.fp16),
)
memory_transformer.down = 
safetensors.torch.load_model(model, f'{checkpoint_root}/---/checkpoint-200000/model.safetensors')
