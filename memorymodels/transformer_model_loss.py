...
def embedding_unreduced_loss(model_output, input_tokens, reduction=False, *args, **kwargs):
    ...
    for i, l in enumerate(loss_per_sample):
        if tokens_per_sample[i] == 1023:
            total_loss += l
            total_tokens += 1
    all_context_losses.append((total_loss / total_tokens).to('cpu').detach())
    if reduction:
...
def clm_unreduced_loss(model_output, input_tokens, reduction=False, *args, **kwargs):
    ...
    for i, l in enumerate(loss_per_sample):
        if tokens_per_sample[i] == 1023:
            total_loss += l
            total_tokens += 1
    all_context_losses.append((total_loss / total_tokens).to('cpu').tolist())
...
if __name__ == '__main__':
    ...
    safetensors.torch.load_model(model, f'{checkpoint_root}/fineweb_training/fineweb_llama_512_n16_h4_c1024/checkpoint-200000/model.safetensors', strict=True) # no decoder_input_embeds param in original model
